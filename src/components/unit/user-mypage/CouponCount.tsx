...
const CouponCount: React.FC<CouponCountProps> = ( { type, availablecoup, usedcoup } ) => {
    const getContent = () => {
        switch(type) {
            case "available":
                return { 
                    icon: <div className='pt-1'><Icon name="couponicon_line" size={35} /></div>, 
                    message: "사용 가능한 쿠폰", 
                    coupnum: availablecoup
                };
            case "used":
                return { 
                    icon: <div className='pt-1.5'><Icon name="usedcouponicon_line" size={30}/></div>, 
                    message: "사용 완료한 쿠폰", 
                    coupnum: usedcoup
                };
        }
